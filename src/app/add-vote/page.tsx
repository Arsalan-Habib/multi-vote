...
export default function AddVote() {
  ...
  const { timeFinished } = useCountdownTimer(votingEndTime);
  ...
  return (
    <div className="container mx-auto pt-8 px-3 max-w-sm relative">
      ...
      <h2 className="text-lg font-semibold mb-8">
        Each member gets 4 votes, and you can only vote for a candidate once.
      </h2>
        ...
        {votes.map((selectedVote, index) => (
          <div key={index} className="mb-6">
            <SelectWithSearch
              title={`Vote #${index + 1}`}
              options={candidates
                .filter((candidate) => !votes.includes(candidate._id) || candidate._id === selectedVote)
                .sort((a, b) => b.votes - a.votes)}
              value={candidates.find((candidate) => candidate._id === selectedVote) || null}
